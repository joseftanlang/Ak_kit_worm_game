...
static void chart_worm_tick() {
  ...
  // Move the worm to the left by a fixed step size, and advance the animation
  // frame counter
...
static void chart_draw_worm() {
  ...
  // Draw the worm's head as a larger circle, and add a simple animation by
  // alternating the vertical position of the head based on the animation frame
  // counter
...
static void chart_screen() {
  ...
  bubble_len = (uint8_t)strlen(
      bubble_text); // Calculate the width of the score bubble based on the
                    // length of the text, with a fixed width per character and
                    // some padding
  bubble_w = (int16_t)(bubble_len * 6 +
                       4); // 6 pixels per character + 4 pixels padding
  bubble_x =
      chart_worm_x -
